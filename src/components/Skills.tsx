...
export const Skills: React.FC = () => {
  ...
  const renderIcon = (iconType: string) => {
    switch (iconType) {
      ...
      case 'GitBranch':
        return <GitBranch className="w-7 h-7 text-orange-400" />;
      case 'Code2':
        return <Code2 className="w-7 h-7 text-blue-400" />;
      case 'Wrench':
        return <Wrench className="w-7 h-7 text-purple-400" />;
      case 'Users':
  ...
  return (
    <section id="skills" className="py-14 relative overflow-hidden bg-[#080b11]/70">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 8 Skills Grid (4 cols on lg, 2 cols on sm) */}
          ...
          {skills.map((skill, index) => (
            <motion.div
              ...
              whileHover={{
                ...
                boxShadow: '0 12px 30px -10px rgba(79, 70, 229, 0.25)'
              }}
              className="p-6 rounded-2xl bg-[#0e1424]/90 border border-white/[0.07] backdrop-blur-md flex flex-col items-start justify-between min-h-[170px] transition-all duration-300 group cursor-default"
            >
              {/* Icon Container */}
              <div className="w-12 h-12 rounded-xl bg-white/[0.04] border border-white/10 flex items-center justify-center mb-4 group-hover:scale-110 group-hover:bg-indigo-950/40 group-hover:border-indigo-500/30 transition-all duration-300">
